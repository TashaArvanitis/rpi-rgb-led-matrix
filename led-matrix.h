// -*- c++ -*-
// Controlling a 32x64 RGB matrix via GPIO.
...
class RGBMatrix {
 public:
  ...
  // Here the set-up  [>] - Only one 32x64 panel
...
private:
  ...
  enum {
    ...
    kColumns = kChainedBoards * 64,
    kPWMBits = 4          // maximum PWM resolution.
  };

  union IoBits {
    // The bits (positioned based on raspberry pi gpio pins)
    struct {
  ...
  // 32x64 RGB is two sub-panels with 64 columns and 16 rows.
  struct DoubleRow {
